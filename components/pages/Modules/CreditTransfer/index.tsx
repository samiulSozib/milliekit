...
import { creditTransferFormSchema, creditTransferListFilterFormSchema } from '@/validation';
...
import type { CreditTransferItemDataType, CreditTransferQueryParamsType, LoggedInDataType } from '@/types';
...
export default function CreditTransferIndex({
  items,
  totalItems,
}: {
  items: CreditTransferItemDataType[];
  totalItems: number;
}) {
  ...
  const transferFormProps = useForm({
    resolver: yupResolver(creditTransferFormSchema),
    mode: 'onBlur',
  ...
  const transferListFilterFormProps = useForm({
    ...
    resolver: yupResolver(creditTransferListFilterFormSchema),
    mode: 'onBlur',
    reValidateMode: 'onSubmit',
  });

  const handleAPIRequest = useCallback(async (data: InferType<typeof creditTransferFormSchema>) => {
    ...
    try {
      const { phone, amount,commission_method } = data;
      await creditTransferRequest.requestToTransfer(
        {
          amount: +amount,
          mobile: phone,
  ...
  const handleFiltersReset = useCallback(() => {
    ...
    router.push(getLocalizedUrl(routeList.creditTransfer.path, lang!));
  }, [lang]);

  const handleSubmittingFilters = useCallback(
    (data: InferType<typeof creditTransferListFilterFormSchema>) => {
      // from-date=2025-02-21&to-date=2025-02-21

      const queryParamsRecord: Partial<CreditTransferQueryParamsType> = { page: 1, per_page: 10 };
      ...
      try {
        router.push(getLocalizedUrl(routeList.creditTransfer.path + '?' + queryParams, lang!));
      } catch (error) {
        console.log(error);
      }
    },
  ...
  useEffect(() => {
    setActiveRoute(routeList.creditTransfer);
  ...
  const { amount: amountFieldErrorData, phone: phoneFieldErrorData, commission_method: commissionMethodFieldErrorData } = transferFormProps.formState.errors;

  return (
    <>
      <PageWrapper name="module-credit-transfer">
        <Header
          title={generalDictionary.creditTransfer}
          wallpaper="/assets/images/modules/credit-transfer/wallpaper.jpg"
                        ...
                        <div className="flex items-center justify-between flex-wrap gap-2">
                          <div className="flex items-center flex-wrap gap-3">
                            ...
                            <div>
                              ...
                              <div className="text-gray-600 text-xsm">{item.mobile}</div>
                            </div>
                          </div>
