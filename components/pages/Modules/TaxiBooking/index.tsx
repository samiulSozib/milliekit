...
import { FilterForm } from '@/components/shared/TaxiFilterForm';
...
export default function TaxiBookingIndex({ latestBookings }: { latestBookings: TaxiTicketRegistrationDataType[] }) {
  ...
  const handleSearch = useCallback((data: InferType<typeof filterFormSchema>) => {
    ...
    try {
      router.push(getLocalizedUrl(routeList.taxiBooking.subPathList.results.path + '?' + queryParams, locale as Locale));
    } catch (error) {
      console.log(error);
    }
  }, []);
