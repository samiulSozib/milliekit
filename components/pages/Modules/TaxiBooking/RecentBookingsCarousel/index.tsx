...
import TaxiTicketCard from '@/components/shared/TaxiTicketCard';
import { SearchForm } from '@/components/shared/TaxiFilterForm';
import { TransactionForm } from '@/components/shared/TaxiTransactionForm';
            ...
            {recentBooks.map((item) => (
              <SwiperSlide key={item.id} className="[&__.ticket-item]:p-[4px]">
                <TaxiTicketCard
                  ...
                  isCancellable={
                    item.booking_status !== 'cancelled' && new Date(item.travel_date).getTime() > Date.now()
                  }
                />
              </SwiperSlide>
            ))}
