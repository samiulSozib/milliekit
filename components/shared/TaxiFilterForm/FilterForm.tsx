...
import { taxiFilterFormSchema } from '@/validation';
...
const FilterFormComponent: FC<FilterFormPropsType> = (props) => {
  ...
  const returnDateGregorianModeValueRef = useRef<Date | null>(null);

  const filterFormProps = useForm({
    resolver: yupResolver(taxiFilterFormSchema),
    mode: 'onBlur',
  ...
  const handleDateChange = useCallback((event: CustomEvent<{ rawDate: Date; calendarDate: string }>) => {
    if (selectedDateType === 'departure') {
      departureDateGregorianModeValueRef.current = event.detail.rawDate;
      filterFormProps.setValue('departure-time', event.detail.calendarDate, {
        shouldValidate: true,
        shouldDirty: true,
      });
    } else {
      returnDateGregorianModeValueRef.current = event.detail.rawDate;
      filterFormProps.setValue('return-time', event.detail.calendarDate, {
        shouldValidate: true,
        shouldDirty: true,
      });
    }
  ...
  }, [filterFormProps, selectedDateType]);
  ...
  const handleChangeAdultPassengers = useCallback(
    ...
    [childPassengerCount, filterFormProps, generalDictionary.passenger]
  );
  ...
  const handleChangeChildPassengers = useCallback(
    ...
    [adultPassengerCount, filterFormProps, generalDictionary.passenger]
  );

  const handleSearch = useCallback(
    (data: InferType<typeof taxiFilterFormSchema>) => {
      ...
      // For round trip, handle return date
      if (tripType === 'round-trip' && returnDateGregorianModeValueRef.current != null) {
        data['return-time'] = i18n.langDateFormatter['en'](_dateOptions).format(
          returnDateGregorianModeValueRef.current
        );
      }

      data['passenger-count'] = adultPassengerCount + childPassengerCount + '';

      // Add trip type to form data
      data['trip-type'] = tripType;

      onSubmit(data);
    },
    [onSubmit, adultPassengerCount, childPassengerCount, tripType]
  );
  ...
  const returnDateInputValue = filterFormProps.watch('return-time') ?? '';

  // Fixed useEffect for date picker
  useEffect(() => {
    let picker: DatePicker | null = null;
    let handler: null | ((event: Event) => void);

    const initializeDatePicker = async () => {
      if (isOpenDatepickerModal && !datePickerRef.current) {
        try {
          const DatePickerModule = await import('@mhf/date-picker/build/date-picker');
          // The date-picker web component should be available now
        } catch (error) {
          console.error('Error loading date picker:', error);
        }
      }

      if (isOpenDatepickerModal) {
        // Use setTimeout to ensure the DOM is updated
        setTimeout(() => {
          picker = datePickerRef.current;
          
          if (picker) {
            // Set the active date based on the selected date type
            const currentValue = selectedDateType === 'departure' ? departureDateInputValue : returnDateInputValue;
            if (currentValue) {
              picker.activeDate = currentValue;
            }

            handler = (event: Event) => {
              handleDateChange(event as CustomEvent<{ rawDate: Date; calendarDate: string }>);
            };
  ...
  }, [isOpenDatepickerModal, selectedDateType, departureDateInputValue, returnDateInputValue, handleDateChange]);

  // Additional useEffect to update date picker when date type changes
  useEffect(() => {
    if (isOpenDatepickerModal && datePickerRef.current) {
      const currentValue = selectedDateType === 'departure' ? departureDateInputValue : returnDateInputValue;
      if (currentValue) {
        datePickerRef.current.activeDate = currentValue;
      }
    }
  }, [isOpenDatepickerModal, selectedDateType, departureDateInputValue, returnDateInputValue]);
  ...
  const onSwapCitiesButtonClick = useCallback(() => {
    ...
    // Also swap form values
    filterFormProps.setValue('origin-city-id', tempDestination.id + '' || '');
    filterFormProps.setValue('destination-city-id', tempOrigin.id + '' || '');
  }, [selectedOrigin, selectedDestination, filterFormProps]);
  ...
  const TripTypeToggle = () => (
    <div className="flex justify-between w-full rounded-lg overflow-hidden">
      <button
        type="button"
        className={`flex-1 px-5 py-2 text-sm font-medium transition-all duration-200 ${
          tripType === "one-way"
  ...
  useEffect(() => {
    locationRequest
      .getCitiesList('af')
      .then((response) => {
        setAllCitiesList(response.items);
      })
      .catch((error) => {
  ...
  }, [filterFormProps, departureDateInputValue, adultPassengerCount, childPassengerCount, generalDictionary.passenger]);

  return (
    <>
      <div className="flex flex-col gap-4">
        {/* Add trip type toggle */}
        <TripTypeToggle />
        ...
        <div className="flex flex-col gap-2 relative">
          ...
          <button
            ...
            aria-label={generalDictionary.swapCities}
          >
            <SwitchIcon width="18" />
          </button>
        </div>

        {/* Date fields layout based on trip type */}
        {tripType === 'one-way' ? (
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <InputTwo
                      ...
                      placeholder={generalDictionary.select}
                      className="font-semibold"
                      label={generalDictionary.returnDate}
                      suffixIcon={<CalenderIcon width="24" className="text-primary cursor-pointer" />}
                      onClick={() => {
        ...
        header={
          <div className="p-5 border-b flex items-center gap-3">
            ...
            <div className="text-lg font-semibold">
              {selectedDateType === 'departure' ? generalDictionary.moveDate : generalDictionary.returnDate}
            </div>
          </div>
        }
      >
